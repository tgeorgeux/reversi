...
socket.on('game_update', function (payload) {
  ...
  $('#my_color').append('<h4>It is ' + payload.game.whose_turn + '\'s turn. Turn timer - <span id="elapsed"></span></h4>');
  clearInterval(interval_timer);
  interval_timer = setInterval(function (last_move_time) {
    return function () {
      /** Do the work of updating the UI */
      var d = new Date();
      var elapsedMilli = d.getTime() - last_move_time;
  ...
  //If board is in Initial state, set AudioCounter to 0
  if(blackSum === 2 && whiteSum === 2){
    AudioCounter = 0;
  }
  //pass update to board after changes are animated
  old_board = board;
  //play sound
  AppendAudioTag()
  if(AudioCounter === 0){
...
function AppendAudioTag(){
    if (AudioCounter === 0){
        $("#audio").html('<audio autoplay="autoplay"><source src="assets/audio/game-start.mp3" type="audio/mpeg" /><embed hidden="true" autostart="true" loop="false" src="misc/your-turn.mp3" /></audio>');     //Play a notification sound!
    }
    else if (AudioCounter === 1){
        $("#audio").html('<audio autoplay="autoplay"><source src="assets/audio/black-turn.mp3" type="audio/mpeg" /><embed hidden="true" autostart="true" loop="false" src="misc/your-turn.mp3" /></audio>');     //Play a notification sound!
    }
    else if (AudioCounter === (-1)){
        $("#audio").html('<audio autoplay="autoplay"><source src="assets/audio/white-turn.mp3" type="audio/mpeg" /><embed hidden="true" autostart="true" loop="false" src="misc/your-turn.mp3" /></audio>');     //Play a notification sound!
    }
    else {
        $("#audio").html('<audio autoplay="autoplay"><source src="assets/audio/tragedy.mp3" type="audio/mpeg" /><embed hidden="true" autostart="true" loop="false" src="misc/your-turn.mp3" /></audio>');     //Play a notification sound!
        console.log('something is wrong with the AudioCounter: Enjoy this sound over and over.')
    }
}